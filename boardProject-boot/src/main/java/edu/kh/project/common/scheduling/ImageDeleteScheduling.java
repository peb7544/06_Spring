...
public class ImageDeleteScheduling {
	...
	//@Scheduled(fixedDelay = 5000) // 이전 작업이 시작된 후 5초에 수행
	...
	//@Scheduled(cron  = " 0 0 * * * *") // 0초 0분 / 정시마다 수행
	//@Scheduled(cron = "0 0 0 * * *") // 자정
	//@Scheduled(cron = "0 0 12 * * *") // 정오
	//@Scheduled(cron= "0,30 * * * * *") // 시계 초 단위가 0, 30인 경우 수행(테스트용)
	...
	public void scheduling() {
		...
		// 배열 내용 복사(깊은 복사)
		// System.arraycopy(복사할배열, 몇번인덱스부터복사할지, 새로운배열, 새로운배열의 몇번부터넣을지인덱스, 복사를 어디까지할건지 길이)
