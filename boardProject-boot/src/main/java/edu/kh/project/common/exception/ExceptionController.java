...
public class ExceptionController {
	...
	// @ExceotuibGabdker(예외 종류)
	
	// 예외 종류 : 메서드별로 처리할 예외를 지정
	//      ex) @ExceptionHandler(SQLException.class) - SQL 관련 예외만 처리
	//          @ExceptionHandler(IDException.class) - 입출력 관련 예외만 처리
	//          @ExceptionHandler(Exception.class) - 모은 예외 처리
	
	@ExceptionHandler(NoResourceFoundException.class) // 404에러 발생시 생기는 오류
