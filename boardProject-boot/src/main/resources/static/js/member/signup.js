...
checkAuthKeyBtn.addEventListener("click", () => {
    if( min === 0 && sec === 0) { // 타이머가 00:00초인 경우
...
memberPw.addEventListener("input", e => {
    ...
    if(inputPw.trim().length === 0) {
        ...
        memberPw.value = ""; // 처음에 띄어쓰기 못하게 하기
        return;
    }
...
memberNickname.addEventListener("input", e => {
    ...
    // 2) 정규식 검사 ( const regExp = /^[가-힣\w\d]{2,10}$/; ) - 수업용공유문서에 있음
    ...
    // - GET요청
    // - 응답 결과 1/0과 같은 COUNT로 할 것
    // - 1: 중복O -> 이미 사용중인 닉네임입니다.(빨간 글씨)
    // - 0: 중복X -> 사용 가능한 닉네임입니다.(초록 글씨)
    fetch("/member/checkNickname?memberNickname=" + inputNickname)
    .then(resp => resp.text())
    .then(count => {
        if(count == 1) {
